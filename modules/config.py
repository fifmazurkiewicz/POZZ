...
def _fetch_secret_json_from_aws(secret_name: str, region_name: Optional[str]) -> Optional[dict]:
    """Fetch secret from AWS Secrets Manager and return as JSON dict if possible, None otherwise.
    
    Returns the parsed JSON dict if the secret is valid JSON, None on error.
    """
    if boto3 is None:
    ...
    # Not JSON, return None (caller should handle raw string separately)
    return None
...
def _fetch_secret_from_aws(secret_name: str, region_name: Optional[str]) -> Optional[str]:
    ...
    using _fetch_secret_json_from_aws() when dealing with JSON secrets.
    """
    if boto3 is None:
        logger.warning("boto3 is not available - cannot fetch secrets from AWS Secrets Manager")
        return None
...
def get_openrouter_api_key() -> Optional[str]:
    ...
    if secret_json and "OPENROUTER_API_KEY" in secret_json:
        key = str(secret_json["OPENROUTER_API_KEY"]).strip()
        logger.info("Successfully retrieved OpenRouter API key from JSON secret")
        return key
...
def get_openai_api_key() -> Optional[str]:
    ...
    secret_name = os.getenv("OPENAI_SECRET_NAME") or os.getenv("OPENROUTER_SECRET_NAME", "POZZ")
    region = os.getenv("AWS_REGION") or os.getenv("AWS_DEFAULT_REGION")
    
    # Try to get JSON first (for multi-field secrets)
    secret_json = _fetch_secret_json_from_aws(secret_name=secret_name, region_name=region)
    if secret_json and "OPENAI_API_KEY" in secret_json:
        key = str(secret_json["OPENAI_API_KEY"]).strip()
        logger.info("Successfully retrieved OpenAI API key from JSON secret")
        return key
...
def get_database_url() -> Optional[str]:
    ...
    # Try to get JSON first (for multi-field secrets like POZZ)
    secret_json = _fetch_secret_json_from_aws(secret_name=secret_name, region_name=region)
    if secret_json:
        # Check for DATABASE_URL first (standard field name)
        if "DATABASE_URL" in secret_json:
            db_url = str(secret_json["DATABASE_URL"]).strip()
            logger.info("Found DATABASE_URL in JSON secret")
            return db_url
        # Fallback: check for OPENROUTER_SECRET_NAME (misnamed field in actual secret)
        if "OPENROUTER_SECRET_NAME" in secret_json:
            db_url = str(secret_json["OPENROUTER_SECRET_NAME"]).strip()
            logger.info("Found DATABASE_URL in OPENROUTER_SECRET_NAME field (fallback)")
            return db_url
        # Fallback: check for POSTGRES_SECRET_NAME
        if "POSTGRES_SECRET_NAME" in secret_json:
            db_url = str(secret_json["POSTGRES_SECRET_NAME"]).strip()
            logger.info("Found DATABASE_URL in POSTGRES_SECRET_NAME field (fallback)")
            return db_url
    
    # Fallback to old method (for backward compatibility with raw string secrets)
