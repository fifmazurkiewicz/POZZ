...
export function ConversationComposer({ draft, onDraftChange, onSend, disabled, placeholder, patientVoice, listening, speaking, onVoiceChange, onListeningChange, inputMode, onInputModeChange, onConversationStart, onConversationStop }: {
  ...
  inputMode: "messages" | "conversation"; onInputModeChange: (value: "messages" | "conversation") => void;
  onConversationStart: () => void; onConversationStop: () => void;
}) {
  const id = useId();
  return <form className="shrink-0 space-y-2 border-t border-[var(--color-divider)] bg-[var(--color-surface)] p-3" onSubmit={onSend}>
    <div className="flex gap-2" role="group" aria-label="Tryb odpowiedzi">
      <button type="button" className={`classical-btn text-sm ${inputMode === "messages" ? "classical-btn-primary" : ""}`} aria-pressed={inputMode === "messages"} onClick={() => onInputModeChange("messages")}>Wiadomości</button>
      <button type="button" className={`classical-btn text-sm ${inputMode === "conversation" ? "classical-btn-primary" : ""}`} aria-pressed={inputMode === "conversation"} onClick={() => onInputModeChange("conversation")}>Rozmowa</button>
    </div>
    {inputMode === "messages" ? <div className="flex gap-2">
      ...
      <button type="button" className="classical-btn classical-btn-primary" disabled={disabled || speaking} onClick={listening ? onConversationStop : onConversationStart}>{listening ? "Zatrzymaj rozmowę" : "Rozpocznij rozmowę"}</button>
      <span className="text-xs text-[var(--color-soft)]" role="status" aria-live="polite">{listening ? "Słucham…" : ""}</span>
    </div>}
    {inputMode === "messages" ? <div className="flex flex-wrap items-center gap-2">
