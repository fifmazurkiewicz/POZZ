...
- **Głos w Symulacji:** chained STT + TTS. Composer ma jedno edytowalne pole tekstowe. Mikrofon nagrywa pojedynczą wypowiedź, a po transkrypcji dopisuje tekst do pola; lekarz może go poprawić i wyłącznie przyciskiem **Wyślij** wysyła turę. Transkrypt + composer są zawsze widoczne i historia automatycznie podąża za nową turą, dopóki użytkownik nie przewinie jej ręcznie.
